...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const qlQuery = all
      ? `objectType = "Building"`
      : `objectType = "Building" AND label like "%${query}%"`
    ...
    if (!resp.ok) {
      ...
      const buildings = (fallbackData.objectEntries ?? []).map((obj: { id: string; objectKey: string; label: string }) => ({
        objectId: obj.id,
    ...
    const buildings = (data.objectEntries ?? []).map((obj: { id: string; objectKey: string; label: string }) => ({
      objectId: obj.id,
      objectKey: obj.objectKey,
      label: obj.label,
    }))
    ...
    console.error('[searchBuildings] Error:', message)
